...
% Custom command for S operator (avoiding conflict with \S section symbol)
...
A \underline{category with attributes} $\langle \mathbb{C}, Att, 1_{\mathbb{C}}, \Sigma, p, \cdot, \S, \#, \gamma, L, \Theta \rangle$ consists of
...
The category of categories with attributes is denoted \underline{Attcat}. A morphism $F : \langle \mathbb{C}, Att, \dots \rangle \to \langle \mathbb{C}', Att', \dots \rangle$ consists of a functor $F : \mathbb{C} \to \mathbb{C}'$ preserving the terminal object and for each object $A$ of $\mathbb{C}$ a function, also called $F$, $F : Att(A) \to Att'(F(A))$ such that $F$, as a whole, preserves all the structure $\Sigma, p, ^*, \S, \#, \gamma, L$ and $\Theta$.

In this section we prove that the category \underline{Attcat} is equivalent to the category \underline{$\Sigma$-Con} of contextual categories with disjoint unions and singleton object. Part of this work has been done in \S 3.1 where we did, in effect, prove that every $\mathbb{D} \in |\underline{\Sigma\text{-con}}|$ induces a category with attributes now to be called $\Phi(\mathbb{D})$. $\Phi(\mathbb{D})$ has as base category the category Base($\mathbb{D}$). If $A \in \text{Base}(\mathbb{D})$ then $Att_{\Phi(\mathbb{D})}(A)$ is taken to be $\{ B \in |\mathbb{D}| \mid A \triangleleft B \text{ in } \mathbb{D} \}$. $\Sigma, p, ^*, \S, \#, L$ and $\Theta$ are then defined in $\Phi(\mathbb{D})$ as defined in \S 3.1. Lemmas of that section then ensure that $\Phi(\mathbb{D})$ so defined is a category with attributes.

Since any morphism $F : \mathbb{D} \to \mathbb{D}'$ in \underline{$\Sigma$-con} completely preserves the disjoint union and singleton object structure as well as contextual structure and since $\Phi(\mathbb{D})$ and $\Phi(\mathbb{D}')$ are defined entirely in terms of this structure it follows that such an $F$ induces a morphism $\Phi(F) : \Phi(\mathbb{D}) \to \Phi(\mathbb{D}')$ in \underline{Attcat}. The functoriality of $\Phi : \underline{\Sigma\text{-con}} \to \underline{\text{Attcat}}$ is immediate. We wish to show that $\Phi$ is an equivalence of categories. This involves the definition of a functor $\Psi : \underline{\text{Attcat}} \to \underline{\Sigma\text{-con}}$.
Then we show that for all $\mathbb{D} \in |\underline{\Sigma\text{-con}}|$, $\Psi(\Phi(\mathbb{D}))$ is, upto isomorphism of structures, the structure $\mathbb{D}$ recovered from $\Phi(\mathbb{D})$.

If $\mathbb{E}$ is a category with attributes write Base($\mathbb{E}$) for the base category of $\mathbb{E}$ and write $Att_{\mathbb{E}}(A)$ for attributes of $A$ when $A \in |\text{Base}(\mathbb{E})|$.

\underline{Construction} If $\mathbb{E}$ is a category with attributes then define a contextual category $\Psi(\mathbb{E})$ as follows:

\underline{Step 1} The objects of $\Psi(\mathbb{E})$ will be defined in such a way that each object is an ordered pair $\langle n, A \rangle$ where $n$ is a natural number and $A$ is an attribute in $\mathbb{E}$.

The tree of objects of $\Psi(\mathbb{E})$ is defined inductively. The least element of the tree is taken to be the ordered pair $\langle 0, L(1_{\mathbb{E}}) \rangle$, where $1_{\mathbb{E}}$ is the terminal object of Base($\mathbb{E}$). Then if $\langle n, A \rangle$ is an object of $\Psi(\mathbb{E})$ define the set of objects of $\Psi(\mathbb{E})$ succeeding $\langle n, A \rangle$ to be the set $\{ \langle n+1, B \rangle \mid B \in Att_{\mathbb{E}}(\Sigma A) \}$.

Thus an arbitrary path up through $\Psi(\mathbb{E})$ is a path of the form $\langle 0, L(1_{\mathbb{E}}) \rangle \triangleleft \langle 1, A_1 \rangle \dots \triangleleft \langle n, A_n \rangle$ where $A_1 \in Att_{\mathbb{E}}(\Sigma L(1_{\mathbb{E}}))$ and for each $i, 2 \le i \le n$, $A_i \in Att_{\mathbb{E}}(\Sigma A_{i-1})$.

\underline{Step 2}. The morphisms of $\Psi(\mathbb{E})$. Define $Hom_{\Psi(\mathbb{E})}(\langle n, A \rangle, \langle m, A' \rangle) = \{ \langle n, m, f \rangle \mid f : \Sigma A \to \Sigma A' \text{ in Base}(\mathbb{E}) \}$. Define $\langle n, m, f \rangle \circ \langle m, l, g \rangle = \langle n, l, fg \rangle$.

\underline{Step 3}. Projections. If $\langle n, A \rangle \triangleleft \langle n+1, B \rangle$ in $\Psi(\mathbb{E})$ then $B \in Att_{\mathbb{E}}(\Sigma A)$ so we can define $p(\langle n+1, B \rangle) = \langle n+1, n, p(B) \rangle$.

\underline{Step 4}. Pullbacks. If $\langle n, m, f \rangle : \langle n, A \rangle \to \langle m, A' \rangle$ in $\Psi(\mathbb{E})$ and if $\langle m, A' \rangle \triangleleft \langle m+1, B \rangle$ then $B \in Att_{\mathbb{E}}(\Sigma A')$ and $f : \Sigma A \to \Sigma A'$ in $\mathbb{E}$ so define $\langle n, m, f \rangle^* \langle m+1, B \rangle = \langle n+1, f^*B \rangle$ and define $q(\langle n, m, f \rangle, \langle m+1, B \rangle) = \langle n+1, m+1, \Sop(f,B) \rangle$. We must check that
\begin{center}
\begin{tikzcd}
\langle n+1, f^*B \rangle \arrow[r, "{\langle n+1{,} m+1{,} \Sop(f{,}B) \rangle}"] \arrow[d] & \langle m+1, B \rangle \arrow[d] \\
\langle n, A \rangle \arrow[r, "{\langle n{,} m{,} f \rangle}"] & \langle m, A' \rangle
\end{tikzcd}
\end{center}
...
\end{tikzcd}
\end{center}
is a pullback diagram in Base($\mathbb{E}$).

Similarly the pullbacks in $\Psi(\mathbb{E})$ fit together because the corresponding pullbacks in Base($\mathbb{E}$) fit together.

\underline{Step 4}. $\Sigma$ and $\alpha$. Suppose that $\langle n, A \rangle \triangleleft \langle n+1, B \rangle \triangleleft \langle n+2, C \rangle$ in $\Psi(\mathbb{E})$. Then $C \in Att_{\mathbb{E}}(\Sigma B)$ and $B \in Att_{\mathbb{E}}(\Sigma A)$ hence $\# C \in Att_{\mathbb{E}}(\Sigma A)$ and $\gamma(C) : \Sigma C \to \Sigma \# C$ in Base($\mathbb{E}$). Thus we can define $\Sigma \langle n+2, C \rangle = \langle n+1, \# C \rangle$ and $\alpha(\langle n+2, C \rangle) = \langle n+2, n+1, \gamma(C) \rangle$. Then $\alpha(\langle n+2, C \rangle)$ is an isomorphism because $\gamma(C)$ is an isomorphism.

The diagram
\begin{center}
\begin{tikzcd}
\langle n+1, C \rangle \arrow[r, "{\alpha(\langle n+2{,} C \rangle)}"] \arrow[dr] & \Sigma \langle n+2, C \rangle \arrow[d] \\
& \langle n, A \rangle
\end{tikzcd}
\end{center}
...
\end{tikzcd}
\end{center}
commutes in Base($\mathbb{E}$).

Finally we must show that if $\langle n, m, f \rangle : \langle n, A \rangle \to \langle m, A' \rangle$ in $\mathbb{C}$ and if $\langle m, A' \rangle \triangleleft \langle m+1, B \rangle \triangleleft \langle m+2, C \rangle$ then $\langle n, m, f \rangle^* \Sigma \langle m+2, C \rangle = \Sigma \langle n, m, f \rangle^* \langle m+2, C \rangle$ and $\langle n, m, f \rangle^* \alpha(\langle m+2, C \rangle) = \alpha(\langle n, m, f \rangle^* \langle m+2, C \rangle)$.

The first identity holds as follows:
$\langle n, m, f \rangle^* \Sigma \langle m+2, C \rangle = \langle n, m, f \rangle^* \langle m+1, \# C \rangle = \langle n+1, f^* \# C \rangle$.
Now by condition III of the definition of Attcat, $f^* \# C = \# \Sop(f,B)^* C$.
Hence $\langle n+1, f^* \# C \rangle = \langle n+1, \# \Sop(f,B)^* C \rangle = \Sigma \langle n+2, \Sop(f,B)^* C \rangle = \Sigma \langle n+1, m+1, \Sop(f,B) \rangle^* \langle m+2, C \rangle = \Sigma q(\langle n, m, f \rangle, \langle m+1, B \rangle)^* \langle m+2, C \rangle = \Sigma \langle n, m, f \rangle^* \langle m+2, C \rangle$ in the extended $^*$ notation.

By lemma 4 of \S 3.1 the second identity is equivalent to the commutativity of
\begin{center}
\begin{tikzcd}
\langle n, m, f \rangle^* \langle m+2, C \rangle \arrow[r, "q"] \arrow[d, "\alpha"] & \langle m+2, C \rangle \arrow[d, "\alpha"] \\
\langle n, m, f \rangle^* \Sigma \langle m+2, C \rangle \arrow[r, "q"] & \Sigma \langle m+2, C \rangle
\end{tikzcd}
\end{center}
If in this diagram the extended $^*, q$ notation is replaced, thus $q(\langle n, m, f \rangle, \langle m+2, C \rangle) = q(q(\langle n, m, f \rangle, \langle m+1, B \rangle), \langle m+2, C \rangle)$ and $\langle n, m, f \rangle^* \langle m+2, C \rangle = q(\langle n, m, f \rangle, \langle m+1, B \rangle)^* \langle m+2, C \rangle$. If $^*, q, \Sigma$ and $\alpha$ are evaluated then the diagram required to commute is
\begin{center}
\begin{tikzcd}
\langle n+2, \Sop(f,B)^* C \rangle \arrow[r, "{\langle n+2{,} m+2{,} \Sop(\Sop(f{,}B){,} C) \rangle}"] \arrow[d, "{\langle n+2{,} n+1{,} \gamma(\Sop(f{,}B)^* C) \rangle}"] & \langle m+2, C \rangle \arrow[d, "{\langle m+2{,} m+1{,} \gamma(C) \rangle}"] \\
\langle n+1, \# \Sop(f,B)^* C \rangle \arrow[r, "{\langle n+1{,} m+1{,} \Sop(f{,} \# C) \rangle}"] & \langle m+1, \# C \rangle
\end{tikzcd}
\end{center}
The commutativity of this diagram follows from the commutativity of the corresponding diagram in Base($\mathbb{E}$) which commutes by condition III of the definition of Attcat.

\underline{Step 5}. Singleton object. Recall that in $\mathbb{E}$ we have $L(1_{\mathbb{E}}) \in Att_{\mathbb{E}}(1_{\mathbb{E}})$ and an isomorphism $\Theta(1_{\mathbb{E}}) : \Sigma L(1_{\mathbb{E}}) \to 1_{\mathbb{E}}$ in Base($\mathbb{E}$). It follows that $\Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}}) \in Att_{\mathbb{E}}(\Sigma L(1_{\mathbb{E}}))$ and thus that $\langle 0, L(1_{\mathbb{E}}) \rangle \triangleleft \langle 1, \Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}}) \rangle$ in $\Psi(\mathbb{E})$. Also since $\Theta(1_{\mathbb{E}})$ is an isomorphism in Base($\mathbb{E}$) it follows that $\Sigma \Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}})$ is isomorphic to $\Sigma L(1_{\mathbb{E}})$. Thus $\Sigma \Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}}) \cong \Sigma L(1_{\mathbb{E}}) \cong 1_{\mathbb{E}}$. Thus $\Sigma \Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}})$ is a terminal object of Base($\mathbb{E}$).

It now follows that $\langle 1, \Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}}) \rangle$ is a terminal object of $\Psi(\mathbb{E})$. We can define the singleton object $\{ \cdot \}$ of $\Psi(\mathbb{E})$ to be $\langle 1, \Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}}) \rangle$.

This completes the description of the contextual category with disjoint unions and singleton object induced by a category with attributes $\mathbb{E}$. Because the morphisms of the categories \underline{Attcat} and \underline{$\Sigma$-con} are in both cases just the structure preserving functions it follows that the above construction is just the object part of a functor $\Psi : \underline{\text{Attcat}} \to \underline{\Sigma\text{-Con}}$.
...
Now define a functor $M : \mathbb{D} \to \text{Base}(\mathbb{D})$ by
\begin{center}
\begin{tikzcd}
A \arrow[d] & \arrow[r, "M"] & \Sigma K A \arrow[d] \\
A' & & \Sigma K A'
\end{tikzcd}
\end{center}
where the morphism on the right is $\alpha^{-1}(K A') \circ \beta(A')^{-1} \circ f \circ \beta(A) \circ \alpha(K A)$.

Finally define a would be contextual functor $\mathbb{F} : \mathbb{D} \to \Psi(\Phi(\mathbb{D}))$ as follows:

If $1 \triangleleft A_1 \dots \triangleleft A_n$ in $\mathbb{D}$ then $\mathbb{F}(A_n) = \langle n, K(A_n) \rangle$.
If $1 \triangleleft A_1 \dots \triangleleft A_n$ and $1 \triangleleft B_1 \dots \triangleleft B_m$ in $\mathbb{D}$ and if $f : A_n \to B_m$ in $\mathbb{D}$ then $\mathbb{F}(f) = \langle n, m, M(f) \rangle$.
...
$\mathbb{F}(C) = \langle n+2, K(C) \rangle$ hence by definition of $\Psi$, $\Sigma \mathbb{F}(C) = \langle n+1, \# K(C) \rangle$ where $\# K(C)$ is calculated in $\Phi(\mathbb{D})$.
...
It remains to show that $\mathbb{F}(\alpha(C)) = \alpha(\mathbb{F}(C))$. Well $\mathbb{F}(\alpha(C))$ is just $\langle n+2, n+1, M(\alpha(C)) \rangle$ and $\alpha(\mathbb{F}(C)) = \alpha(\langle n+2, K(C) \rangle) = \langle n+2, n+1, \gamma(K(C)) \rangle$ where $\gamma(K(C))$ is calculated in $\mathbb{D}$ as
...
\begin{lemma}
If $\mathbb{G} : \mathbb{D} \to \mathbb{D}'$ is a morphism in \underline{$\Sigma$-Con} then the diagram
\begin{center}
\begin{tikzcd}
...
\end{tikzcd}
\end{center}
commutes in \underline{$\Sigma$-Con}.
\end{lemma}
...
This completes the demonstration that $\Phi \circ \Psi \cong id_{\underline{\Sigma\text{-Con}}}$.
For the remainder of this section we aim at proving that $\Psi \circ \Phi \cong id_{\underline{\text{Attcat}}}$ and thus that \underline{$\Sigma$-Con} and \underline{Attcat} are equivalent categories. Throughout we suppose that $\mathbb{E}$ is a category with attributes. Eventually we show that $\Phi(\Psi(\mathbb{E})) \cong \mathbb{E}$.

If $A \in |\text{Base}(\mathbb{E})|$ then $\Theta(1_{\mathbb{E}})^* L(A) \in Att_{\mathbb{E}}(\Sigma L(1_{\mathbb{E}}))$ and $\Sop(\Theta(1_{\mathbb{E}}), L(A)) \circ \Theta(A) : \Sigma \Theta(1_{\mathbb{E}})^* L(A) \to A$ is an isomorphism. We define $J(A) = \Theta(1_{\mathbb{E}})^* L(A)$ and $\Pi(A) = \Sop(\Theta(1_{\mathbb{E}}), L(A)) \circ \Theta(A)$ then for each $A \in |\text{Base}(\mathbb{E})|$, $J(A) \in Att_{\mathbb{E}}(\Sigma L(1_{\mathbb{E}}))$ and $\Pi(A) : \Sigma J(A) \to A$ is an isomorphism in Base($\mathbb{E}$). We can then define a functor $M : \text{Base}(\mathbb{E}) \to \text{Base}(\mathbb{E})$ by
\begin{center}
\begin{tikzcd}
...
We shall eventually define an isomorphism $\mathcal{J} : \mathbb{E} \to \Phi(\Psi(\mathbb{E}))$ in terms of $M$ and $\Pi$ but we will come back to that later. Lemmas 3-9 just function to show that $\mathcal{J}$ once defined, is a morphism.

\begin{lemma}
If $A$ is an object of Base($\mathbb{E}$) and if $B \in Att_{\mathbb{E}}(A)$ then
$\#(\Sop(\Theta(1_{\mathbb{E}}), L(A))^* \Theta(A)^* B) = \Theta(1_{\mathbb{E}})^* \#(\Theta(A)^* B)$ and
$\gamma(\Pi(A)^* B) \circ \Pi(\Sigma B) = \Sop(\Sop(\Theta(1_{\mathbb{E}}), L(A)), \Theta(A)^* B) \circ \gamma(\Theta(A)^* B)$ in Base($\mathbb{E}$).
\end{lemma}
...
$= \gamma^{-1}(\Pi(A)^* B) \circ p(\Pi(A)^* B) \circ \Pi(A) \circ \Pi^{-1}(A)$ because of commutivity of the pullback diagram for $\S$ along $\Pi(A)$.
$= \gamma^{-1}(\Pi(A)^* B) \circ p(\Pi(A)^* B)$ as required.
\end{proof}

\begin{lemma}
If $f: A \to A'$ in Base($\mathbb{E}$) and $B \in Att_{\mathbb{E}}(A')$ then
$\Pi(\Sigma f^* B) \circ \Sop(f,B) \circ \Pi^{-1}(\Sigma B) = \gamma^{-1}(\Pi(A)^* f^* B) \circ \Sop(\Pi(A) \circ f \circ \Pi^{-1}(A'), \Pi(A')^* B) \circ \gamma(\Pi(A')^* B)$.
\end{lemma}

\begin{proof}
$\Pi(\Sigma f^* B) \circ \Sop(f,B) \circ \Pi^{-1}(\Sigma B) = \Sop(\Theta(1_{\mathbb{E}}), L(\Sigma f^* B)) \circ \Theta(f^* B) \circ \Sop(f,B) \circ \Theta^{-1}(\Sigma B) \circ \Sop(\Theta(1_{\mathbb{E}}), \Theta(1_{\mathbb{E}})^* L(\Sigma B))$, by def of $\Pi$.
$= \Sop(\Theta(1_{\mathbb{E}}), L(\Sigma f^* B)) \circ \gamma^{-1}(\Theta(A)^* f^* B) \circ \Sop(\Theta(A), f^* B) \circ \Sop(f,B) \circ \Sop(\Theta^{-1}(A'), \Theta(A')^* B) \circ \gamma(\Theta(A')^* B) \circ \Sop(\Theta^{-1}(1_{\mathbb{E}}), \Theta(1_{\mathbb{E}})^* L(\Sigma B))$, using condition IV of attcat.
$= \gamma^{-1}(\Pi(A)^* f^* B) \circ \Sop(\Sop(\Theta(1_{\mathbb{E}}), L(A)), \Theta(A)^* f^* B) \circ \Sop(\Theta(A), f^* B) \circ \Sop(f,B) \circ \Sop(\Theta^{-1}(A'), \Theta(A')^* B) \circ \Sop(\Sop(\Theta^{-1}(1_{\mathbb{E}}), \Theta(1_{\mathbb{E}})^* L(A')), \Pi(A')^* B) \circ \gamma(\Pi(A')^* B)$, by lemma 3.
$= \gamma^{-1}(\Pi(A)^* f^* B) \circ \Sop(\Pi(A) \circ f \circ \Pi^{-1}(A'), \Pi(A')^* B) \circ \gamma(\Pi(A')^* B)$, by fitting the pullbacks together.
\end{proof}

\begin{lemma}
If $A$ is an object of Base($\mathbb{E}$) and if $B \in Att_{\mathbb{E}}(A)$ and $C \in Att_{\mathbb{E}}(\Sigma B)$ then $\Pi(A)^* \# C = \# \gamma(\Pi(A)^* B)^* \Pi(\Sigma B)^* C$ and
$\Pi(\Sigma C) \circ \gamma(C) \circ \Pi^{-1}(\Sigma \# C) = \gamma^{-1}(\Pi(\Sigma B)^* C) \circ \Sop(\gamma(\Pi(A)^* B), \gamma(\Pi(A)^* B)^* \Pi(\Sigma B)^* C) \circ \gamma(\#(\gamma(\Pi(A)^* B)^* \Pi(\Sigma B)^* C)) \circ \Sop(\gamma(\#(\Pi(A)^* B)^* \Pi(\Sigma B)^* C) \circ \# \gamma(\Pi(A)^* B)^* \Pi(\Sigma B)^* C)$.
\end{lemma}

\begin{proof}
Use condition III of attcat at $\Pi(A) : \Sigma J A \to A$ and $B \in Att_{\mathbb{E}}(A), C \in Att_{\mathbb{E}}(\Sigma B)$. We get $\Pi(A)^* \# C = \# \Sop(\Pi(A), B)^* C$ and
\begin{center}
\begin{tikzcd}
\Sigma \Sop(\Pi(A), B)^* C \arrow[r, "{\Sop(\Sop(\Pi(A){,} B){,} C)}"] \arrow[d, "{\gamma(\Sop(\Pi(A){,} B)^* C)}"'] & \Sigma C \arrow[d, "\gamma(C)"] \\
\Sigma \Pi(A)^* \# C \arrow[r, "{\Sop(\Pi(A){,} \# C)}"] & \Sigma \# C
\end{tikzcd}
\end{center}
commutes.

The first identity follows from lemma 4 which says that $\gamma(\Pi(A)^* B) \circ \Pi(\Sigma B) = \Sop(\Pi(A), B)$.

The R.H.S. of the second identity is simplified by the same lemma. It becomes $\gamma^{-1}(\Pi(\Sigma B)^* C) \circ \Sop(\gamma(\Pi(A)^* B), \Sop(\Pi(A), B)^* C) \circ \gamma(\# \Sop(\Pi(A), B)^* C)$. Using both the above identities from condition III the R.H.S. becomes $\gamma^{-1}(\Pi(\Sigma B)^* C) \circ \Sop(\gamma^{-1}(\Pi(A)^* B) \circ \Sop(\Pi(A), B), C) \circ \gamma(C) \circ \Sop(\Pi^{-1}(A), \Pi(A)^* \# C) \circ \gamma(\Pi(A)^* \# C)$.
Use lemma 4 twice and we get the L.H.S.
\end{proof}

\begin{lemma}
If $A$ is an object of Base($\mathbb{E}$) then
...
\begin{proof}
Both $p(\Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}}))$ and $\Sop(\Theta(1_{\mathbb{E}}), L(1_{\mathbb{E}}))$: $\Sigma J(1_{\mathbb{E}}) \to \Sigma L(1_{\mathbb{E}})$ in Base($\mathbb{E}$). Since $\Sigma L(1_{\mathbb{E}})$ is terminal, $p(\Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}})) = \Sop(\Theta(1_{\mathbb{E}}), L(1_{\mathbb{E}}))$. Now the R.H.S. of required identity becomes $\gamma^{-1}(\Pi(1_{\mathbb{E}})^* L(A)) \circ \Sop(\Sop(\Theta(1_{\mathbb{E}}), L(1_{\mathbb{E}})), \Theta(1_{\mathbb{E}})^* L(A))$.
Using lemma 3 on LHS we get $(\Theta(1_{\mathbb{E}}), L \Sigma L A) \circ \Theta(\Sigma L A) \circ \Theta(A) \circ \Theta^{-1}(A) \circ \Sop(\Theta^{-1}(1_{\mathbb{E}}), \Theta(1_{\mathbb{E}})^* L A)$.

Using the same lemma 3 on R.H.S. we get $\Sop(\Theta(1_{\mathbb{E}}), L \Sigma L A) \circ \gamma^{-1}(\Theta(1_{\mathbb{E}})^* L A)$.

Now LHS = RHS follows from the case of condition IV: $\gamma^{-1}(\Theta(1_{\mathbb{E}})^* L A) = \Theta(\Sigma L A) \circ \Theta(\Theta^{-1}(1_{\mathbb{E}}), \Theta(1_{\mathbb{E}})^* L A)$.
\end{proof}

Before we can define $\mathcal{J} : \mathbb{E} \to \Phi(\Psi(\mathbb{E}))$ we must know what $\Phi(\Psi(\mathbb{E}))$ looks like. We know what the $*, q, \Sigma, \alpha$ structure of $\Psi(\mathbb{E})$ is. The $\S, \#, \gamma, L, \Theta$ structure is defined in terms of this and induces the category with attributes $\Phi(\Psi(\mathbb{E}))$. The next lemma calculates the effects of the definition of $\S, \#, \gamma, L, \Theta$ in $\Psi(\mathbb{E})$.

\begin{lemma}
If $\mathbb{E}$ is a category with attributes then
\begin{enumerate}
    \item[i.] If $\langle 1, 1, f \rangle : \langle 1, A \rangle \to \langle 1, A' \rangle$ and $\langle 1, A' \rangle \triangleleft \langle 2, B \rangle$ in $\Psi(\mathbb{E})$ then $\Sop(\langle 1, 1, f \rangle, \langle 2, B \rangle) = \langle 1, 1, \gamma^{-1}(f^* B) \circ \Sop(f, B) \circ \gamma(B) \rangle$.
    \item[ii.] If $1 \triangleleft \langle 1, A \rangle \triangleleft \langle 2, B \rangle$ and $\Sigma \langle 2, B \rangle \triangleleft \langle 2, C \rangle$ in $\Psi(\mathbb{E})$ then $\# \langle 2, C \rangle = \langle 2, \# \gamma(B)^* C \rangle$ and $\gamma(\langle 2, C \rangle) = \langle 1, 1, \gamma^{-1}(C) \circ \Sop(\gamma^{-1}(B), \gamma(B)^* C) \circ \gamma(\gamma(B)^* C) \circ \gamma(\# \gamma(B)^* C) \rangle$.
    \item[iii.] If $1 \triangleleft \langle 1, A \rangle$ in $\Psi(\mathbb{E})$ then $L(\langle 1, A \rangle) = \langle 2, p(\Theta^{-1}(1_{\mathbb{E}})^* L(1_{\mathbb{E}}))^* A \rangle$ and $\Theta(\langle 1, A \rangle) = \langle 1, 1, \gamma^{-1}(p(\Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}}))^* A) \circ \Sop(p(\Theta(1_{\mathbb{E}})), A) \rangle$.
\end{enumerate}
\end{lemma}

\begin{proof}
i. $\Sop(\langle 1, 1, f \rangle, \langle 2, B \rangle) = \alpha^{-1}(\langle 1, 1, f \rangle^* \langle 2, B \rangle) \circ q(\langle 1, 1, f \rangle, \langle 2, B \rangle) \circ \alpha(\langle 2, B \rangle)$, by definition of $\Phi$.
$= \alpha^{-1}(\langle 2, f^* B \rangle) \circ \langle 2, 2, \Sop(f, B) \rangle \circ \alpha(\langle 2, B \rangle)$ by def of $\Psi$.
$= \langle 1, 2, \gamma^{-1}(f^* B) \rangle \circ \langle 2, 2, \Sop(f, B) \rangle \circ \langle 2, 1, \gamma(B) \rangle$, by def. of $\Psi$.
$= \langle 1, 1, \gamma^{-1}(f^* B) \circ \Sop(f, B) \circ \gamma(B) \rangle$.
\end{proof}

Cases ii. and iii. are just as straightforward.

Given the definition of $\Phi$ we can now give a complete description of $\Phi(\Psi(\mathbb{E}))$ as follows:

$|\text{Base}(\Phi(\Psi(\mathbb{E})))| = \{ \langle 1, A \rangle \mid A \in Att_{\mathbb{E}}(\Sigma L(1_{\mathbb{E}})) \}$
$Hom_{\Phi(\Psi(\mathbb{E}))}(\langle 1, A \rangle, \langle 1, A' \rangle) = \{ \langle 1, 1, f \rangle \mid f: \Sigma A \to \Sigma A' \text{ in Base}(\mathbb{E}) \}$
$Att_{\Phi(\Psi(\mathbb{E}))}(\langle 1, A \rangle) = \{ \langle 2, B \rangle \mid B \in Att_{\mathbb{E}}(\Sigma A) \}$
$1_{\Phi(\Psi(\mathbb{E}))} = \langle 1, \Theta^{-1}(1_{\mathbb{E}})^* L(1_{\mathbb{E}}) \rangle$
If $\langle 2, B \rangle \in Att_{\Phi(\Psi(\mathbb{E}))}(\langle 1, A \rangle)$ then $\Sigma \langle 2, B \rangle = \langle 1, \# B \rangle$ and $p(\langle 2, B \rangle) = \langle 1, 1, \gamma^{-1}(B) \circ p(B) \rangle$.
If $\langle 2, B \rangle \in Att_{\Phi(\Psi(\mathbb{E}))}(\langle 1, A \rangle)$ and if $\langle 2, C \rangle \in Att_{\Phi(\Psi(\mathbb{E}))}(\Sigma \langle 2, B \rangle)$ then $\# \langle 2, C \rangle = \langle 2, \# \gamma(B)^* C \rangle$ and $\gamma(\langle 2, C \rangle) = \langle 1, 1, \gamma^{-1}(C) \circ \Sop(\gamma^{-1}(B), \gamma(B)^* C) \circ \gamma(\gamma(B)^* C) \circ \gamma(\# \gamma(B)^* C) \rangle$.
If $\langle 1, 1, f \rangle : \langle 1, A \rangle \to \langle 1, A' \rangle$ in Base($\mathbb{E}$) and if $\langle 2, B \rangle \in Att_{\Phi(\Psi(\mathbb{E}))}(\langle 1, A' \rangle)$ then $\langle 1, 1, f \rangle^* \langle 2, B \rangle = \langle 2, f^* B \rangle$ and $\Sop(\langle 1, 1, f \rangle, \langle 2, B \rangle) = \langle 1, 1, \gamma^{-1}(f^* B) \circ \Sop(f, B) \circ \gamma(B) \rangle$.
If $\langle 1, A \rangle \in |\text{Base}(\Phi(\Psi(\mathbb{E})))|$ then $L(\langle 1, A \rangle) = \langle 2, p(\Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}}))^* A \rangle$ and $\Theta(\langle 1, A \rangle) = \langle 1, 2, \gamma^{-1}(p(\Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}}))^* A) \circ \Sop(p(\Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}})), A) \rangle$.

Define $\mathcal{J} : \mathbb{E} \to \Phi(\Psi(\mathbb{E}))$ by i. if $A$ is an object of Base($\mathbb{E}$) then $\mathcal{J}(A) = \langle 1, \Theta(1_{\mathbb{E}})^* L(A) \rangle$. ii. If $f: A \to A'$ in Base($\mathbb{E}$) then $\mathcal{J}(f) = \langle 1, 1, \Pi(A) \circ f \circ \Pi^{-1}(A') \rangle$. iii. If $B \in Att_{\mathbb{E}}(A)$ then $\mathcal{J}(B) = \langle 2, \Pi(A)^* B \rangle$.

$\mathcal{J}$ is easily seen to be 1-1 and onto from objects, morphisms and attributes of $\mathbb{E}$ to objects, morphisms, respectively attributes of $\Phi(\Psi(\mathbb{E}))$. $\mathcal{J}$ preserves $\Sigma$ and $p$ by virtue of lemma 3. $^*$ is easily seen to be preserved. $\S$ is preserved by $\mathcal{J}$, use lemma 4. $\#$ and $\gamma$ are preserved, use lemma 5.
Terminal object easily seen to be preserved. $L$ preserved because $1_{\mathbb{E}}$ terminal implies $(\Theta(1_{\mathbb{E}})^* L(1_{\mathbb{E}})) \circ \Theta(1_{\mathbb{E}}) = \Pi(1_{\mathbb{E}})$. $\Theta$ preserved use lemma 6.

Finally, the collection of isomorphisms $\{ \mathcal{J}_{\mathbb{E}} \mid \mathbb{E} \in |\underline{\text{Attcat}}| \}$ cannot help but be natural in $\mathbb{E}$. Thus we have a natural isomorphism $\Phi \circ \Psi \cong id_{\underline{\text{Attcat}}}$, completing the proof that $\underline{\Sigma\text{-Con}} \cong \underline{\text{Attcat}}$.

\section{Categories and Fibrations}
...
By far the easiest way of describing the structure is by describing the corresponding category with attributes which is to be called \underline{Fib}. The significance of this structure is twofold. In the first place the most attractive notion of a category indexed family of categories does lead to a contextual category and thus generalised algebraic theories can be interpreted in this manner.

Further more this notion is not reducible to any morphism with codomain notion and so there is no parallel interpretation of essentially algebraic theories.
...
The category with attributes \underline{Fib} of categories indexing categories is described as follows:

\begin{enumerate}
    \item Base(\underline{Fib}) = \underline{Cat}, the category of all (small) categories.
    \item If $A$ is a category then an attribute of type $A$ is an $A$-indexed family of categories, i.e. is a functor $B : A \to \underline{Cat}$.
    \item If $B \in Att_{\underline{Fib}}(A)$, i.e. if $B : A \to \underline{Cat}$ is a functor, then $\Sigma B$ is the category defined as follows:
    \[ |\Sigma B| = \{ \langle a,b \rangle \mid a \in |A| \text{ and } b \in |B(a)| \}. \]
    \[ Hom_{\Sigma B}(\langle a,b \rangle, \langle a',b' \rangle) = \{ \langle f,g \rangle \mid f : a \to a' \text{ in } A \text{ and } g : B(f)(b) \to b' \text{ in } B(a') \}. \]
    If $\langle a,b \rangle \in |\Sigma B|$ then $id_{\langle a,b \rangle} = \langle id_a, id_b \rangle$.
    If $\langle a,b \rangle \xrightarrow{\langle f,g \rangle} \langle a',b' \rangle \xrightarrow{\langle f',g' \rangle} \langle a'',b'' \rangle$ in $\Sigma B$ then $\langle f,g \rangle \circ \langle f',g' \rangle = \langle f \circ f', B(f')(g) \circ g' \rangle$.
    
    $p(B) : \Sigma B \to A$ is defined to be the 1st projection functor.

    \item If $F : A \to A'$ in \underline{Cat} and if $B \in Att_{\underline{Fib}}(A')$ then $F^*B \in Att_{\underline{Fib}}(A)$ is defined to be the functor $B \circ F : A \to \underline{Cat}$. $\Sop(F,B) : \Sigma F^*B \to \Sigma B$ is defined to be the functor "Apply $F$ to the 1st component leave other component unchanged". For example, if $\langle a,b \rangle \in |\Sigma F^*B|$ then $a \in |A|$ and $b \in |B(F(a))|$, hence $\langle F(a), b \rangle \in |\Sigma B|$. Similarly on morphisms of $\Sigma F^*B$.

    \item If $B : A \to \underline{Cat}$ and $C : \Sigma B \to \underline{Cat}$ then define $\# C : A \to \underline{Cat}$ as follows:
    If $a \in |A|$ then $\# C(a)$ is the category such that
    \[ |\# C(a)| = \{ \langle b,c \rangle \mid b \in |B(a)| \text{ and } c \in |C(\langle a,b \rangle)| \} \]
    \[ Hom_{\# C(a)}(\langle b,c \rangle, \langle b',c' \rangle) = \{ \langle g,h \rangle \mid g : b \to b' \text{ in } B(a) \text{ and } h : C(\langle a,g \rangle)(c) \to c' \text{ in } C(\langle a,b' \rangle) \}. \]
    
    If $f : a \to a'$ in $A$ then $\# C(f)$ is the functor $\# C(a) \to \# C(a')$ given by
    \begin{center}
    \begin{tikzcd}
    \langle b,c \rangle \arrow[d, "{\langle g{,}h \rangle}"'] \arrow[r, "\# C(f)"] & \langle B(f)(b), C(\langle f, id_{B(f)(b)} \rangle)(c) \rangle \arrow[d, "{\langle B(f)(g){,} C(\langle f{,} id_{B(f)(b')} \rangle)(h) \rangle}"] \\
    \langle b',c' \rangle \arrow[r] & \langle B(f)(b'), C(\langle f, id_{B(f)(b')} \rangle)(c') \rangle
    \end{tikzcd}
    \end{center}
    ...
    \[ \langle \langle f,g \rangle, h \rangle \mapsto \langle f, \langle g,h \rangle \rangle \]

    \item Chose $1_{\underline{Fib}}$ to be any terminal object of \underline{Cat}. If $A$ is a category then $L(A) \in Att_{\underline{Fib}}(1)$ is taken to be the functor : $1 \to \underline{Cat}$ whose value at the unique object of $1$ is $A$. Then $\Sigma L(A) = 1 \times A$.
    Define $\Theta(A) : 1 \times A \to A$ to be the projection functor.
\end{enumerate}

That completes the description of \underline{Fib}. Checking that \underline{Fib} is a category with attributes is very straightforward and rather tedious.

\section{Martin-Lof Type Theory}

Martin-Lof type theory is a generalisation of the typed $\lambda$-calculus. It is more general in that in the syntax there are variable types - the structure of substitution is of the general form. The algebraic semantics of Martin-Lof type theory is thus provided by a extension of the theory of contextual categories. This extension we call the theory of weak M-L structures. Every weak M-L structure is a model of Martin-Lof type theory. In fact the definition of weak M-L structure is a set theoretic definition of the notion 'model of Martin-Lof type theory'. It is the most general possible such definition.

We need to explain that Martin-Lof type theory generalises a weak version of the typed $\lambda$-calculus. The $\eta$-rule of $\lambda$-calculus, $s = \lambda x . Ap(x,s)$, corresponding to the uniqueness of the term $\lambda x.t$ subject to the condition $Ap(x, \lambda x.t) = t$, is not assumed. Neither is the rule $pr(p_1(z), p_2(z)) = z$ assumed where $pr$ is the pairing function and $p_1$ and $p_2$ are the projections. On the other hand cartesian closed categories correspond to a strong version of the typed $\lambda$-calculus which includes these two rules. The effect of all this is that whereas cartesian closedness is definable in terms of universal arrows, weak M-L structure is defined in terms of weak universal arrows (the definition of weak universal arrow is like the definition of universal arrow except that the uniqueness condition is dropped). If we strengthen the definition of weak M-L structure by replacing weak universality by universality, then we can drop the adjective weak and call the structures strong M-L structures. The strong M-L structures provide us with the algebraic semantics of a strengthened version of M-L type theory, stronger just by the inclusion of a rule expressing the uniqueness of $\lambda x.t$ subject to the condition $Ap(x, \lambda x.t) = t$ and similar uniqueness conditions, one for each logical scheme.

In the following definition of strong M-L structure we use the same notation as Martin-Lof uses except in the case of notation for the type with precisely one element - Martin-Lof uses the notation $N_1$ for this type but we use the notation $\{ \cdot \}$. Our use of $\Sigma$ and $\{ \cdot \}$ notation in this section will be consistent with our use of the same notation for disjoint unions and singleton object in previous sections.

A \underline{strong M-L structure} $\langle \mathbb{C}, \Sigma, pr, \Pi, Ap, Id, r, +, i, j, \{ \cdot \}, e, N, o, S \rangle$ consists of a contextual category $\mathbb{C}$ and the following additional structure:
...
\begin{enumerate}
    ...
    \end{tikzcd}
    \end{center}
    commutes and such that $Id(A)$ and $r(A)$ have the following property: for every object $C$ of $\mathbb{C}$ such that $Id(A) \triangleleft C$, for every morphism $h: A \to C$ such that the diagram
    \begin{center}
    \begin{tikzcd}
        & C \arrow[d] \\
        h \arrow[ur] & Id(A) \\
    ...
    \item[iv.] Whenever $Q \triangleleft A$ and $Q \triangleleft B$ in $\mathbb{C}$, an object $A+B$ of $\mathbb{C}$ such that $Q \triangleleft A+B$ and morphisms $i_{A,B} : A \to A+B$ and $j_{A,B} : B \to A+B$ such that the diagrams
    \begin{center}
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
        A \arrow[r, "{i_{A{,}B}}"] \arrow[dr] & A+B \arrow[d] \\
        & Q
    \end{tikzcd}
    \end{minipage}
    and
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
        B \arrow[r, "{j_{A{,}B}}"] \arrow[dr] & A+B \arrow[d] \\
        & Q
    \end{tikzcd}
    \end{minipage}
    \end{center}
    commute and such that $A+B, i_{A,B}, j_{A,B}$ have the following property: for every object $C$ of $\mathbb{C}$ such that $A+B \triangleleft C$, for all pairs of morphisms $h_1: A \to C$ and $h_2: B \to C$ such that the diagrams
    \begin{center}
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
        & C \arrow[d] \\
        A \arrow[ur, "h_1"] \arrow[r, "{i_{A{,}B}}"'] & A+B
    \end{tikzcd}
    \end{minipage}
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
        & C \arrow[d] \\
        B \arrow[ur, "h_2"] \arrow[r, "{j_{A{,}B}}"'] & A+B
    \end{tikzcd}
    \end{minipage}
    \end{center}
    commute, there exists a unique $g \in Arr_{\mathbb{C}}(C)$ such that in the diagram
    \begin{center}
    \begin{tikzcd}
        & C \\
        & g \arrow[u] \\
        A \arrow[uur, "h_1", bend left] \arrow[r, "{i_{A{,}B}}"] & A+B \arrow[u] & B \arrow[l, "{j_{A{,}B}}"'] \arrow[uul, "h_2"', bend right]
    \end{tikzcd}
    \end{center}
    both triangles commute.

    \item[v.] An object $\{ \cdot \}$ of $\mathbb{C}$ such that $1 \triangleleft \{ \cdot \}$ and a morphism $e : 1 \to \{ \cdot \}$ in $\mathbb{C}$ having the following property: for all objects $Q$ of $\mathbb{C}$, for all objects $A$ of $\mathbb{C}$ such that $p(Q,1)^* \{ \cdot \} \triangleleft A$ in $\mathbb{C}$, for all morphisms $f: Q \to A$ in $\mathbb{C}$ such that the diagram
    \begin{center}
    \begin{tikzcd}
        & A \arrow[d] \\
        f \arrow[ur] & p(Q,1)^* \{ \cdot \} \\
        Q \arrow[ur] &
    \end{tikzcd}
    \end{center}
    commutes, there exists a unique $g \in Arr_{\mathbb{C}}(A)$ such that the diagram
    \begin{center}
    \begin{tikzcd}
        & A \\
        f \arrow[ur] & g \arrow[u] \\
        Q \arrow[ur] & p(Q,1)^* \{ \cdot \} \arrow[u] \\
        & p(Q,1)^* e \arrow[u]
    \end{tikzcd}
    \end{center}
    commutes.

    \item[vi.] An object $N$ of $\mathbb{C}$ such that $1 \triangleleft N$ and morphisms $o: 1 \to N$ and $S: N \to N$ in $\mathbb{C}$ having the following property: for all objects $Q$ of $\mathbb{C}$, for all objects $A$ of $\mathbb{C}$ such that $p(A,1)^* N \triangleleft A$, for all morphisms $a: Q \to A$ and $b: A \to A$ such that the diagrams
    \begin{center}
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
        & A \arrow[d] \\
    ...
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
        A \arrow[r, "b"] \arrow[d] & A \arrow[d] \\
        p(Q,1)^* N \arrow[r, "{p(Q{,}1)^* S}"] & p(Q,1)^* N
    \end{tikzcd}
    \end{minipage}
    \end{center}
    commute, there exists a unique $f \in Arr_{\mathbb{C}}(A)$ such that the diagrams
    \begin{center}
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
        A \arrow[r, "f"] \arrow[d] & A \arrow[d] \\
        p(Q,1)^* N \arrow[r, "{p(Q{,}1)^* o}"] & p(Q,1)^* N
    \end{tikzcd}
    \end{minipage}
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
        A \arrow[r, "b"] \arrow[d, "f"'] & A \arrow[d, "f"] \\
        p(Q,1)^* N \arrow[r, "{p(Q{,}1)^* S}"] & p(Q,1)^* N
    \end{tikzcd}
    \end{minipage}
    \end{center}
...
Clauses i. ... vi. in the definition of strong M-L structure correspond to the schemes for $\Sigma, \Pi, Id, +, N_1$ and $N$ in Martin-Lof type theory. If the word unique is dropped from any of these clauses then what remains is an exact rewriting of the corresponding Martin-Lof scheme within the language of contextual categories.

The final condition says that $f^*$ always preserves $\Sigma, pr, Ap$ etc.. Thus it says that the contextual functor $\mathbb{C}_f : \mathbb{C}_{Q'} \to \mathbb{C}_Q$ is a structure preserving morphism whenever $f: Q \to Q'$ in $\mathbb{C}$. It is, then, the `substitution is a homomorphism between algebras of terms' condition. This is an implicit property of syntactical substitution which must always be stated explicitly in any algebraic semantics. That this is a property of syntactical substitution is because if we substitute into any expression that is the result of binding simpler expressions by a logical symbol then the result is identical with the result of first substituting into the simpler expressions and then binding by the logical symbol.

This is work which syntax always does for us and which algebra has got to be made to do. Hence the condition.

Not surprisingly the contextual category \underline{Fam} of sets, families of sets and so on is in a natural way a strong M-L structure. $\Sigma$ is calculated by taking actual disjoint unions of families of sets, $\Pi$ calculated by taking cartesian products of families of sets. $Id(A)$, when $A$ is a set, is the characteristic family of the identity predicate on $A$. $+$ is interpreted by coproducts and $N$ is taken to be the set of natural numbers.

The definition of strong M-L structure can be simplified considerably. In the first place those parts of the definition that are about $\Sigma, pr, \{ \cdot \}$ and $e$ are equivalent to $\langle \mathbb{C}, \Sigma, pr, \{ \cdot \}, e \rangle$ being a contextual category with disjoint unions and singleton object. This leads to simplifications of the other clauses. Clause ii. can be used to simplify clause vi. It turns out, then, that the definition that we have given is equivalent to the following definition.

A \underline{strong M-L structure} $\langle \mathbb{C}, \Sigma, pr, \Pi, Ap, Id, r, +, i, j, \{ \cdot \}, e, N, o, S \rangle$ consists of a contextual category with disjoint unions and singleton object $\langle \mathbb{C}, \Sigma, pr, \{ \cdot \}, e \rangle$ and the following additional structure.
...
\begin{enumerate}
    ...
    \end{tikzcd}
    \end{center}
    commutes and having the property: for all $h \in Arr_{\mathbb{C}}(B)$, there exists a unique $g \in Arr_{\mathbb{C}}(\Pi B)$ such that the diagram
    \begin{center}
    \begin{tikzcd}
        p(A)^* \Pi B \arrow[r, "Ap(B)"] & B \\
    ...
    $A \xrightarrow{i_{A,B}} A+B \xleftarrow{j_{A,B}} B$ is a coproduct diagram in Base($\mathbb{C}_Q$).

    \item[vi'.] An object $N$ of $\mathbb{C}$ such that $1 \triangleleft N$ and morphisms $o: 1 \to N$, $S: N \to N$ such that $\langle N, o, S \rangle$ is a natural number object in AugBase($\mathbb{C}$). ($\langle N, o, S \rangle$ is a natural number object in the category $\mathcal{G}$ with terminal object $1$ iff $o: 1 \to N$ and $S: N \to N$ and for all objects $A$ of $\mathcal{G}$, for all morphisms $a: 1 \to A$ and $b: N \to N$, there exists a unique morphism $f: N \to A$ such that
    \begin{center}
    \begin{tikzcd}
        N \arrow[r, "f"] & A \\
        o \arrow[u] & a \arrow[u] \\
...
An \underline{M-L Hyperdoctrine} $\langle \mathbb{E}, \Sigma, \Pi, Id, r, +, i, j, N, o, S \rangle$ consists of a category with attributes $\mathbb{E}$ with the following additional structure:

\begin{enumerate}
    \item[i.] For every object $A$ of Base($\mathbb{E}$), for every $B \in Att_{\mathbb{E}}(A)$, for every $C \in Att_{\mathbb{E}}(\Sigma B)$, an attribute $\Pi C \in Att_{\mathbb{E}}(A)$ and a morphism $\tau(C) : \Sigma p(B)^* \Pi C \to \Sigma C$ in Base $\mathbb{E}$ such that the diagram
    \begin{center}
    \begin{tikzcd}
    ...
    \end{tikzcd}
    \end{center}
    commutes and having the property that for all morphisms $f: \Sigma B \to \Sigma C$ in Base($\mathbb{E}$) such that $p(C) \circ f = id_{\Sigma B}$, there exists a unique $g: A \to \Sigma \Pi C$ in Base $\mathbb{E}$ such that $p(\Pi C) \circ g = id_A$ and such that the diagram
    \begin{center}
    \begin{tikzcd}
    ...
    \item[ii.] For all objects $A$ of Base($\mathbb{E}$), for all $B \in Att_{\mathbb{E}}(A)$, an attribute $Id B \in Att_{\mathbb{E}}(\Sigma p(B)^* B)$ and a morphism $r(B): \Sigma B \to \Sigma Id B$ in Base $\mathbb{E}$ such that the diagram
    \begin{center}
    \begin{tikzcd}
    ...
    \end{tikzcd}
    \end{center}
    commutes (where $\Delta(B)$ is the unique morphism such that $\Delta(B) \circ p(p(B)^* B) = id$ and $\Delta(B) \circ \Sop(p(B), B) = id$) and having the property that for all attributes $C \in Att_{\mathbb{E}}(\Sigma p(B)^* B)$ and for all morphisms $h: \Sigma B \to \Sigma C$ such that the diagram
    \begin{center}
    \begin{tikzcd}
    ...
    \item[iii.] For all objects $A$ of Base($\mathbb{E}$), for all $B_1, B_2 \in Att(A)$, an object $B_1+B_2 \in Att(A)$ and morphisms $i_{B_1, B_2} : \Sigma B_1 \to \Sigma(B_1+B_2)$ and $j_{B_1, B_2} : \Sigma B_2 \to \Sigma(B_1+B_2)$ such that the diagrams
    \begin{center}
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
    ...
    \end{minipage}
    \end{center}
    commute and having the property that for all $C \in Att_{\mathbb{E}}(A)$, for all pairs of morphisms $h_1: \Sigma B_1 \to \Sigma C$ and $h_2: \Sigma B_2 \to \Sigma C$ such that the diagrams
    \begin{center}
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
    ...
    \item[iv.] An object $N$ of Base($\mathbb{E}$), morphisms $o: 1 \to N$ and $S: N \to N$ such that $\langle N, o, S \rangle$ is a natural number object in Base($\mathbb{E}$).
\end{enumerate}

Subject to the conditions: If $f: A \to A'$ in Base $\mathbb{E}$, if $B \in Att_{\mathbb{E}}(A')$ and $C \in Att_{\mathbb{E}}(\Sigma B)$ then $f^* \Pi C = \Pi \Sop(f,B)^* C$ and the diagram
\begin{center}
\begin{tikzcd}
...
\begin{enumerate}
    \item[vi.] If $f: A \to A'$ in Base($\mathbb{E}$) and if $B \in Att_{\mathbb{E}}(A')$ then $\Sop(\Sop(f,B), p(B)^* B)^* Id B = Id f^* B$ and the diagram
    \begin{center}
    \begin{tikzcd}
    ...
    \item[vii.] If $f: A \to A'$ in Base($\mathbb{E}$) and $B_1, B_2 \in Att_{\mathbb{E}}(A')$ then $f^*(B_1+B_2) = f^*B_1 + f^*B_2$ and the diagrams
    \begin{center}
    \begin{minipage}{0.4\textwidth}
    \begin{tikzcd}
...
We are claiming, then, that the category of M-L structures and the category of M-L hyperdoctrines are equivalent. The proof of this result is an extension of the proof that \underline{$\Sigma$-Con} and \underline{Attcat} are equivalent categories.

\section{Limit Spaces - a model of Martin-Lof type theory}

We wish to describe a model of Martin-Lof type theory in which types are interpreted as limit spaces and in which families of types indexed by a type are, roughly speaking, interpreted as 'morphisms with codomain' in the category of limit spaces. The model is described as an M-L hyperdoctrine with base category the category of limit spaces. If we first describe the M-L hyperdoctrine of sets and families of sets then the M-L hyperdoctrine of limit spaces can be described without much trouble.

But first, two very useful and trivial lemmas:

\begin{lemma}
If $\mathbb{E}$ is a category with attributes, if $A$ is an object of Base($\mathbb{E}$), If $B \in Att_{\mathbb{E}}(A)$, if $D \in Att_{\mathbb{E}}(\Sigma p(B)^* B)$ and if $r: \Sigma B \to \Sigma D$ is an isomorphism in Base($\mathbb{E}$) such that the diagram
\begin{center}
\begin{tikzcd}
...
\end{tikzcd}
\end{center}
commutes then $\langle D, r \rangle$ satisfies clause ii. of the definition of M-L hyperdoctrine. That is clause ii. is satisfied if $Id(B)$ is taken to $D$ and if $r(B)$ is taken to be $r$.
\end{lemma}

\begin{lemma}
If $\mathbb{E}$ is a category with attributes, if $A$ is an object of $\mathbb{E}$, If $B_1, B_2 \in Att_{\mathbb{E}}(A)$, If $C \in Att_{\mathbb{E}}(A)$ and if $i: \Sigma B_1 \to \Sigma C$, $j: \Sigma B_2 \to \Sigma C$ in Base($\mathbb{E}$) such that the diagrams
\begin{center}
\begin{minipage}{0.4\textwidth}
\begin{tikzcd}
...
\end{minipage}
\end{center}
commute and if $\Sigma B_1 \xrightarrow{i} \Sigma C \xleftarrow{j} \Sigma B_2$ is a coproduct diagram in Base($\mathbb{E}$) then $\langle C, i, j \rangle$ satisfy clause iii. of the definition of M-L hyperdoctrine. That is if $B_1+B_2$ is taken to be $C$ and if $i_{B_1, B_2}$ is taken to be $i$ and similarly if $j_{B_1, B_2}$ is taken to be $j$ then clause iii. is satisfied.
\end{lemma}

The M-L hyperdoctrine \underline{Fam} of sets and families of sets is as follows:

\begin{enumerate}
    \item Base(\underline{Fam}) = \underline{Set}.
    \item If $A \in |\text{Base}(\underline{Fam})|$, i.e. if $A$ is a set, then $Att_{\underline{Fam}}(A) = \{ A\text{-indexed families of sets} \}$.
    \item If $A \in |\text{Base}(\underline{Fam})|$ and if $B \in Att(A)$ then $\Sigma B = \bigcup_{a \in A} B(a) = \{ \langle a,b \rangle \mid a \in A \text{ and } b \in B(a) \}$.
    \item If $f: A \to A'$ in Base(\underline{Fam}) if $B \in Att(A')$ then $f^*B = \lambda a \in A . B(f(a))$. $\Sop(f,B) : \Sigma f^*B \to \Sigma B$ is given by $\Sop(f,B)(\langle a,b \rangle) = \langle f(a), b \rangle$.
    \item If $A \in |\text{Base}(\underline{Fam})|$, if $B \in Att(A)$, if $C \in Att(\Sigma B)$ then $\# C \in Att(A)$ is given by $\# C(a) = \{ \langle b,c \rangle \mid b \in B(a) \text{ and } c \in C(\langle a,b \rangle) \}$. $\gamma(C) : \Sigma \# C \to \Sigma C$ is given by $\gamma(C)(\langle a, \langle b,c \rangle \rangle) = \langle \langle a,b \rangle, c \rangle$.
    \item If $A \in |\text{Base}(\underline{Fam})|$ then $L(A) = \lambda x \in \{ \cdot \} . A$. Then $\Theta(A) : \Sigma L A \to A$ is given by $\Theta(A)(\langle \cdot, a \rangle) = a$.
    \item If $A \in |\text{Base}(\underline{Fam})|$, If $B \in Att(A)$, if $C \in Att(\Sigma B)$ then define $\Pi C \in Att(A)$ by
    \[ \Pi C(a) = \prod_{x \in B(a)} C(\langle a,x \rangle). \]
    If $\Pi C$ is so defined then $\Sigma p(B)^* \Pi C = \{ \langle \langle a,b \rangle, g \rangle \mid a \in A, b \in B(a) \text{ and } g \in \prod_{x \in B(a)} C(\langle a,x \rangle) \}$.
    Define $Ap(C) : \Sigma p(B)^* \Pi C \to \Sigma C$ by defining
    \[ Ap(C)(\langle \langle a,b \rangle, g \rangle) = \langle \langle a,b \rangle, Ap(b,g) \rangle. \]
    \item If $A \in |\text{Base}(\underline{Fam})|$ and if $B \in Att(A)$ then $\Sigma p(B)^* B = \{ \langle \langle a,b \rangle, b' \rangle \mid a \in A \text{ and } b,b' \in B(a) \}$. Define $Id(B)$, $r(B)$ by
    $Id(B)(\langle \langle a,b \rangle, b' \rangle) = \{ \cdot \}$ if $b=b'$, $= \emptyset$ otherwise.
    $r(B)(\langle a,b \rangle) = \langle \langle \langle a,b \rangle, b \rangle, \cdot \rangle$.
    \item If $A \in |\text{Base}(\underline{Fam})|$, If $B_1, B_2 \in Att(A)$ then define $B_1+B_2$ by $(B_1+B_2)(a) = B_1(a) + B_2(a)$. Then $\Sigma(B_1+B_2) = \Sigma B_1 + \Sigma B_2$. Define $i_{B_1, B_2}$ and $j_{B_1, B_2}$ accordingly.
    \item As is well known the set of natural numbers is a natural number object in the category \underline{Set}.
\end{enumerate}

Completing the description of \underline{Fam} as an M-L hyperdoctrine.
...
If $f: A \to A'$ is a function then define $\mathcal{F}(f) : \mathcal{F}(A) \to \mathcal{F}(A')$ by $\mathcal{F}(f)(\Phi) = \{ w \subseteq A' \mid \exists u \in \Phi \text{ s.t. } w \supseteq f(u) \}$. Then $\mathcal{F}(f)$ is an order preserving map and it is easy to see that $\mathcal{F}$ is functorial: $\mathcal{F}(f \circ f') = \mathcal{F}(f) \circ \mathcal{F}(f')$ and $\mathcal{F}(id_A) = id_{\mathcal{F}(A)}$. $\mathcal{F}$ can be considered to be a functor from \underline{Set} to the category of ordered sets.

A \underline{limit space} $\langle A, C \rangle$ consists of a set $A$ and for each $a \in A$, a set $C(a) \subseteq \mathcal{F}(A)$, said to be the set of filters converging to $a$, and subject to the conditions:

i. If $a \in A$ then $\langle a \rangle \in C(a)$, where $\langle a \rangle$ is the principle filter generated by $a$.

ii. If $a \in A$ and $\Phi, \Psi \in C(a)$ then $\Phi \cap \Psi \in C(a)$.

iii. If $a \in A$ and $\Phi \in C(a)$ and if $\Phi \subseteq \Psi$ in $\mathcal{F}(A)$ then $\Psi \in C(a)$.

We usually say that $\Phi \text{ conv } a$ in preference to $\Phi \in C(a)$.

A morphism of limit spaces $f: \langle A, C \rangle \to \langle A', C' \rangle$ is a function $f: A \to A'$ such that whenever $\Phi \text{ conv } a$ in $\langle A, C \rangle$ then $\mathcal{F}(f)(\Phi) \text{ conv } f(a)$ in $\langle A', C' \rangle$.

The category of limit spaces will be denoted \underline{Limsp}.

We shall make use of the fact that for every $f: A \to A'$ in \underline{Set}, the morphism $\mathcal{F}(f) : \mathcal{F}(A) \to \mathcal{F}(A')$ has a left adjoint. We define an order preserving morphism $\check{\mathcal{F}}(f) : \mathcal{F}(A') \to \mathcal{F}(A)$ by $\check{\mathcal{F}}(f)(\Phi) = \{ u \subseteq A \mid \exists w' \in \Phi \text{ s.t. } u \supseteq f^{-1}(w') \}$.
...
\begin{proof}
1. $\check{\mathcal{F}}(f)(\Phi) \subseteq \Theta \Rightarrow \Phi \subseteq \mathcal{F}(f)(\Theta)$. Assume that $\check{\mathcal{F}}(f)(\Phi) \subseteq \Theta$. Then $u \in \Phi \Rightarrow f^{-1}(u) \in \check{\mathcal{F}}(f)(\Phi) \Rightarrow f^{-1}(u) \in \Theta \Rightarrow f(f^{-1}(u)) \in \mathcal{F}(f)(\Theta)$. Hence $u \in \Phi \Rightarrow u \in \mathcal{F}(f)(\Theta)$ because $f(f^{-1}(u)) \subseteq u$. Thus $\check{\mathcal{F}}(f)(\Phi) \subseteq \Theta \Rightarrow \Phi \subseteq \mathcal{F}(f)(\Theta)$.

2. The converse. Suppose $\Phi \subseteq \mathcal{F}(f)(\Theta)$. Suppose $v \in \check{\mathcal{F}}(f)(\Phi)$. Then $\exists u \in \Phi$ such that $f^{-1}(u) \subseteq v$. $u \in \Phi \Rightarrow u \in \mathcal{F}(f)(\Theta)$. Hence $u \supseteq f(w)$, for some $w \in \Theta$. But $u \supseteq f(w)$ iff $f^{-1}(u) \supseteq w$. Hence $v \in \Theta$. As required, $\Phi \subseteq \mathcal{F}(f)(\Theta) \Rightarrow \check{\mathcal{F}}(f)(\Phi) \subseteq \Theta$.
\end{proof}

\begin{corollary}
(a) $\check{\mathcal{F}} : \underline{Set}^{op} \to$ the category of ordered sets, is a functor.
(b) For each $f: A \to A'$, $\check{\mathcal{F}}(f) : \mathcal{F}(A') \to \mathcal{F}(A)$ preserves meets.
\end{corollary}

The base category of the M-L hyperdoctrine of limit spaces is taken to be the category \underline{Limsp}. Then if $\mathcal{A} \in |\underline{\text{Limsp}}|$ and if $\mathcal{A}$ has underlying set $A$ then define $Att(\mathcal{A}) = \{ \langle B, C \rangle \mid B \text{ is an A-indexed family of sets and } C \text{ is such that } \langle \Sigma B, C \rangle \text{ is a limit space such that } p(B) : \langle \Sigma B, C \rangle \to \mathcal{A} \text{ is continuous} \}$.
If $\mathcal{B} \in Att(\mathcal{A})$ then define $\Sigma \mathcal{B} = \langle \Sigma B, C \rangle$, where $B$ is the underlying family of sets associated with $\mathcal{B}$. Define $p(\mathcal{B}) = p(B)$. Thus every $\mathcal{B} \in Att(\mathcal{A})$ is determined by an A-indexed family of sets $B$ and a limit space $\Sigma \mathcal{B}$ which has underlying set $\Sigma B$ and which is such that $p(B) : \Sigma \mathcal{B} \to \mathcal{A}$ is continuous.

\underline{Pullbacks} If $f: \mathcal{A} \to \mathcal{A}'$ and if $\mathcal{B} \in Att(\mathcal{A}')$ then the underlying family of sets of $f^* \mathcal{B}$ is taken to be $f^*B$. Then $\Sigma f^* \mathcal{B}$ is taken to be the unique limit space with underlying set $\Sigma f^* B$ such that
\begin{center}
\begin{tikzcd}
...
\end{tikzcd}
\end{center}
is a pullback diagram in \underline{Limsp}.

Convergence in $\Sigma f^* \mathcal{B}$ is given by $\Theta \text{ conv } x$ iff $\mathcal{F}(p(f^*B))(\Theta) \text{ conv } p(f^*B)(x)$ and $\mathcal{F}(\Sop(f,B))(\Theta) \text{ conv } \Sop(f,B)(x)$. That the diagram is a pullback diagram follows immediately from the fact that the underlying diagram in \underline{Set} is a pullback diagram. Uniqueness of this convergence relation subject to the diagram being a pullback diagram is the case because if $X$ is a new limit space with underlying set $\Sigma f^* B$ and such that
\begin{center}
\begin{tikzcd}
X \arrow[r, "{\Sop(f{,}B)}"] \arrow[d, "p(f^* \mathcal{B})"'] & \Sigma \mathcal{B} \arrow[d, "p(\mathcal{B})"] \\
\mathcal{A} \arrow[r, "f"] & \mathcal{A}'
\end{tikzcd}
\end{center}
is a pullback diagram then there exists an isomorphism $g: \Sigma f^* \mathcal{B} \to X$ such that $g \circ p(f^* \mathcal{B}) = p(f^* \mathcal{B})$ and $g \circ \Sop(f,B) = \Sop(f,B)$. But then since the corresponding diagrams in \underline{Set} are pullback diagrams so $g = id_{\Sigma f^*B}$. So $id_{\Sigma f^*B}$ is morphism of limit spaces $id_{\Sigma f^*B} : \Sigma f^* \mathcal{B} \to X$ and also $id_{\Sigma f^*B} : X \to \Sigma f^* \mathcal{B}$. Hence $\Phi \text{ conv } x$ in $\Sigma f^* \mathcal{B}$ implies $\mathcal{F}(id_{\Sigma f^*B})(\Phi) \text{ conv } x$ in $X$ i.e. $\Phi \text{ conv } x$ in $X$. And vice versa. Thus $\Sigma f^* \mathcal{B}$ is unique.
...
\underline{$\#$ and $\gamma$.} If $\mathcal{A} \in |\underline{\text{Limsp}}|$, If $\mathcal{B} \in Att(\mathcal{A})$ and If $\mathcal{C} \in Att(\Sigma \mathcal{B})$ then the underlying family of $\# \mathcal{C}$ is taken to be $\# C$. The limit space $\Sigma \# \mathcal{C}$ is then uniquely determined by the requirement that $\gamma(C) : \Sigma \mathcal{C} \to \Sigma \# \mathcal{C}$ be an isomorphism of limit spaces.

\underline{L and $\Theta$.} \underline{Limsp} has a terminal object $1$. If $\mathcal{A} \in |\underline{\text{Limsp}}|$ then $L(\mathcal{A})$ is taken to have underlying family of sets $L(A)$ and $\Sigma L(\mathcal{A})$ is then determined by the requirement that $\Theta(A) : \Sigma L(\mathcal{A}) \to \mathcal{A}$ be an isomorphism of limit spaces.

\underline{Id, + and N} are all trivial to define. For Id and + lemmas 1 and 2 of this section can be used.

\underline{$\Pi$ and Ap} The slightly non trivial bit. If $\mathcal{A} \in |\underline{\text{Limsp}}|$, If $\mathcal{B} \in Att(\mathcal{A})$ and if $\mathcal{C} \in Att(\Sigma \mathcal{B})$ then define $\Pi \mathcal{C}(a) = \{ f \in \prod_{x \in B(a)} C(\langle a,x \rangle) \mid \text{the corresponding } f: a^* \mathcal{B} \to \Sop(a, \mathcal{B})^* \mathcal{C} \text{ is continuous} \}$.
...
\begin{enumerate}
    \item Barendregt, H. P. \quad Some extensional models for combinatory logics and $\lambda$-calculi, thesis, Utrecht, 1971
    
    \item Benabou, J. \quad Introduction to bicategories, Springer LNM 47, 1971
    
    \item Birkhoff, G. and Lipson, J. D. \quad Heterogeneous algebras, J. Combinatory Theory 8, 1970
    
    \item Fourman, M. P. \quad Higher order intuitionistic logic and topoi, thesis, Oxford 1974
    
    \item Freyd, P. \quad Aspects of topoi, Bull. Austral. Math. Soc. 7, 1972
    
    \item Hyland, M. \quad Recursion theory on the countable functionals, thesis, Oxford, 1975
    
    \item Gray, J. W. \quad The categorical comprehension scheme, Springer LNM 99, 1969
    
    \item Gray, J. W. \quad Formal category theory: adjointness for 2-categories, Springer LNM 391, 1974
    
    \item Kelly, G. M. \quad (Editor) Category seminar, Springer LNM 420, 1974
    
    \item Kelly, G. M. and Street, R. \quad Review of the elements of 2-categories, in (9)
    
    \item Lawvere, F. W. \quad Functorial Semantics of algebraic theories, Proc. Nat. Acad. Sci. 50, 1963
    
    \item Lawvere, F. W. \quad Functorial Semantics of elementary theories, abstract, J. of symbolic logic 31, 1966
    
    \item Lawvere, F. W. \quad Theories as categories and the completeness theorem, abstract, J. of symbolic logic 32, 1967
    
    \item Lawvere, F. W. \quad Some algebraic problems in the context of functorial semantics of algebraic theories, Springer LNM 99, 1968
    
    \item Lawvere, F. W. \quad Adjointness in foundations, Dialectica 23, 1968
    
    \item Lawvere, F. W. \quad Equality in hyperdoctrines and comprehension schema as an adjoint functor. Proc. AMS Symposia Pure Math. 17, 1970
    
    \item Lawvere, F. W. \quad Introduction, Model theory and topoi, Springer LNM 445, 1975
    
    \item Lambek, J. \quad Deductive systems and categories I, Math. Systems Theory 2, 1968
    
    \item Lambek, J. \quad Deductive systems and categories II, Springer LNM 86, 1969
    
    \item Lambek, J. \quad Deductive systems and categories III, Springer LNM 274, 1972
    
    \item Linton, F. E. J. \quad Some aspects of equational theories, Proc. conf. categorical algebra, La Jolla (1965), Springer 1966
    
    \item Linton, F. E. J. \quad An outline of functorial semantics, Springer LNM 80, 1969
    
    \item MacLane, S. \quad Categories for the working mathematician, Springer-Verlag, 1972
    
    \item MacLane, S. \quad (Editor) Coherence in categories, Springer LNM 281, 1972.
    
    \item Martin-Lof, P. \quad An intuitionistic theory of types, Logic Colloquium '73, North Holland
    
    \item Myers, A. \quad Cartesian closed categories and $\lambda$-calculus, dissertation, Bristol 1974
    
    \item Pareigis, B. \quad Categories and functors, Acad. Press, 1970
    
    \item Reyes, G. \quad Sheaves and concepts, to appear
    
    \item Szabo, M. E. \quad Algebra of proofs, North Holland, 1976
\end{enumerate}
